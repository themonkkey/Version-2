...
RE_PARTICIPANT_COUNT = re.compile(
    ...
    # an adjective sits between the number and the noun as often as not:
    # "28 departmental officials were present from both sectors"
    r"|(\d{1,4})\s+(?:[a-z]+\s+){0,2}Officials?\s+were\s+present"
...
def parse_report_b(rep, display_name):
    ...
    return {
        ...
        # lvl_src is the CARD's own claim (its Activity row, else its table);
        # the body cues are read from the CARD's own lines. Both halves matter:
        # a format-B page carries several districts' cards, and the banner names
        # everything the whole day covered, so either the neighbouring block or
        # the banner would otherwise decide this card's level.
...
def build_district(stem, display_name, dkey, write_images=True):
    ...
    return {
        ...
        # Count what the SESSIONS actually carry, not what was lifted from the
        # PDF: a duplicate report is merged away above and a format-B card can be
        # another district's, so len(photos) over-counts both. The strip on the
        # page and the rails under the cards have to agree.
        "photos_total": sum(len(s["photos"]) for s in sessions),
